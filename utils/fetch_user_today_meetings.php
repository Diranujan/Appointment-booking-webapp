...
if (isset($_GET['whereBy']) && isset($_GET['whereValue'])) {
    ...
    if (isset($_GET['date'])) {
        ...
        $sql = "SELECT 
                ...
                WHERE
                    user_meetings.`$whereBy`= '$whereValue' AND meetings.pref_date = '$date' AND meetings.status='$status'";
        ...
        echo json_encode($todaymeeting);
    } else {
        echo json_encode(array("error" => "No Meeting Today Book yet."));
        
    }
} else {
    echo json_encode(array("error" => "could not find the user"));
}
